...
- Upstream: the current `synthetic-sciences/openscience` default branch is
  `main` at `e9844a49f1f4d93cbf5f88b8f4880c003adc6e61` (2026-07-11).
- Its root `LICENSE` is Apache-2.0. Its root `NOTICE` identifies OpenScience,
  Copyright 2026 Synthetic Sciences, and records markitdown, scientific data
  source, and client-rendering notices. Spark retains the applicable license,
  notice, source path, pinned revision, and modification record for adaptations.
- Spark Foundation baseline verified here: `0adc592`. General Research uses
  pinned OpenCode 1.17.13 (`10c894bdeef3618f5666fb506ef7f9491bb964d8`).
...
| General research agent | `backend/cli/src/agent/agent.ts`, `backend/cli/src/agent/prompt/research.txt` | App-private OpenCode profile + desktop session | adapted / Apache-2.0 | OpenCode owns the session and loop. Runtime | Foundation | partial | Profile, SDK, desktop tests, deterministic live agent/tool smoke |
...
| Research workflow | `backend/cli/src/agent/prompt/research.txt` | Research agent prompt and workspace | adapted / Apache-2.0 | Preserve iterative scope/reason/compute/analyze/write behavior. Runtime | Foundation | partial | Prompt contract + deterministic vertical smoke |
...
| Permission presets | `backend/cli/src/permission/next.ts` (behavior reference) | Native pinned-OpenCode config, `OPENCODE_PERMISSION` overlay, resolved `/agent` validation, desktop switch | behavior-only / Spark | Balanced is the manual default. Autonomous Research — Developer Preview allows ordinary project edits, analysis, web, task, Skill, and curated paper metadata reads; destructive, global/system install, upload, credential, custom MCP, and external-directory operations remain ask/deny. Stored `full` is migration compatibility only. Neither mode is Full Access or process isolation. Runtime/Desktop | Batch 2 | partial | 246 Rust policy/migration/resolved-agent tests, 679 Desktop tests, profile contract, deterministic pinned-OpenCode live smoke; packaged switch and real-provider run remain unverified |
| General sandbox | `backend/cli/src/sandbox/sandbox.ts` | No selectable General Research sandbox | excluded / no source reuse | Remove the non-functional UI placeholder; do not claim isolation. Desktop | Foundation | excluded | Controls test asserts the Sandbox option is absent |
...
| Foundation scientific skills | `backend/cli/skills/{research,coding,writing,visualization}/**` | `runtime/skills/` | behavior-only / MIT | Ship 29 independently written, loadable research/data/writing skills; upstream paths document behavioral references only. Skills | Batch 2 | parity | Manifest/frontmatter/name/reference validation, profile tests, and pinned OpenCode discovery/invocation |
| Web search | agent tool permissions | OpenCode web/MCP tools when configured | behavior-only / OpenCode | Balanced asks. Autonomous allows ordinary OpenCode web search/fetch plus curated credential-free paper metadata tools; downloads, custom MCP, uploads, paid, credential, and external operations retain approval boundaries. Runtime | Batch 2 | partial | Native preset/resolved-agent tests and profile contract; real multi-source provider run unverified |
| File tools | agent tool permissions | OpenCode workspace tools | behavior-only / OpenCode | Both presets deny external-directory access and ask for sensitive reads. Balanced asks for mutations; Autonomous allows ordinary project edits but not destructive or external changes. Runtime | Batch 2 | partial | Profile contract + native preset and resolved-agent tests |
| Shell | agent tool permissions | OpenCode shell tool | behavior-only / OpenCode | Balanced asks for shell. Autonomous allows ordinary project analysis and project-local dependency work while delete, global/system installs, uploads, credentials, remote/paid actions, and policy bypasses ask/deny. This is not process isolation. Runtime | Batch 2 | partial | Native command-pattern and resolved-agent tests; Balanced live one-time approval smoke |
| Python | research prompt + coding skills | Workspace Python through OpenCode plus `.spark/python` | behavior-only / Spark | The UI can bootstrap a pinned per-project environment through bundled uv; ordinary Autonomous project Python can run without Docker. Hard arbitrary-code isolation and automatic pre-turn bootstrap remain open. Runtime | Batch 2 | partial | Rust bootstrap/symlink/snapshot tests and deterministic CSV/PNG smoke; real project-env Agent run unverified |
| Notebook | `src/science/kernel/**`, notebook UI | Persistent local Python/R kernels, app-managed JupyterLab, project Python, and shell `nbconvert` | behavior-only / Spark | The Research prompt can create and execute `.ipynb` through the managed environment, and the current viewer discovers executed notebooks. Agent Jupyter MCP remains fail-closed; deeper live-cell Agent control and a real create/execute/open E2E remain open. Runtime | Batch 2 | partial | Dependency, kernel, notebook serialization/discovery, credential, readiness, and path-validation tests |
...
| Scientific writing | `backend/cli/src/agent/prompt/write.txt`, `backend/cli/skills/writing/**` | Write agent + scientific-writing skill | adapted / Apache-2.0 | Keep claims, citations, limitations, and uncertainty explicit. Skills | Foundation | in-progress | Agent/skill profile tests |
...
## Batch 2 autonomous evidence

Batch 2 implements the developer-preview path but does not claim that every
autonomous scenario has been exercised by a real model. The default Composer now
sends one selected Research Agent/model turn; OpenCode owns planning and native
task children, while the deterministic planner is an optional Manual Parallel
Tasks action. The runtime has 29 validated core Skills, a project-Python setup
path, literature normalization outputs, notebook execution instructions, native
child observation, and a post-turn artifact summary.

| Scenario | Evidence status |
| --- | --- |
| A — Data | Implemented, not fully exercised: prompt, project environment, artifacts, and real-provider harness exist; intentional failure/repair was not run. |
| B — Literature | Implemented, not fully exercised: mode-aware specialist permissions, curated paper reads, two-source bundle normalization, CSV/BibTeX/report outputs; no live two-source model run. |
| C — Mixed PDF + CSV | Prompt and artifact path implemented; no execution-level smoke. |
| D — Native delegation | Deterministic pinned-OpenCode smoke proves one child result and restart lineage; two-or-more-child real-model behavior is unverified. |
| E — Notebook | Managed packages, `nbconvert` path, serialization, discovery, and viewer exist; create/execute/open E2E is unverified. |
| F — Restart | Deterministic smoke proves session, child lineage, history, and artifacts; research-state recovery is not separately exercised. |
| G — Permissions | Rust tests prove ordinary Autonomous tools plus destructive/custom-MCP/system guards; a complete Autonomous live no-approval run is unverified. |

`pnpm test:autonomous:live` is opt-in. On 2026-07-17 it reported an explicit
skip because `SPARK_LIVE_MODEL` and `SPARK_LIVE_API_KEY` were absent. No real
provider/model quality claim is made. The deterministic `test:foundation:live`
run passed against pinned OpenCode 1.17.13 and remains separate evidence.
