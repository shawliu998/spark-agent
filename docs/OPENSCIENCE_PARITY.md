...
| Permission presets | `backend/cli/src/permission/next.ts` (behavior reference) | Native pinned-OpenCode config, `OPENCODE_PERMISSION` overlay, resolved `/agent` validation, desktop switch | behavior-only / Spark | Balanced is default and asks for edits, all shell, web, MCP, and unknown tools. Full Access is workspace-oriented: edit/patch may allow, but shell, web, MCP, install, remote, and delete still ask. Both deny `external_directory`; neither is approval-off or process isolation. Runtime/Desktop | Foundation | partial | Rust policy/migration/resolved-agent tests, profile contract, desktop store/control tests; packaged switch not run |
