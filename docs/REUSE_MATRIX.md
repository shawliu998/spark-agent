...
| K-Dense scientific Skills | Existing Spark core pack has 29 loadable Skills and validation; upstream offers a much larger collection | Upstream root reports MIT, but individual Skill metadata still requires validation | `implement-missing-gap` | Add a pinned, curated manifest/deployment adapter through the existing OpenCode Skill directory; never execute upstream scripts or install every dependency. | Adapter in progress; bundled pack remains fallback |
